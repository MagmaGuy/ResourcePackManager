...
public class CommandManager implements CommandExecutor, TabCompleter {
    ...
    public CommandManager(JavaPlugin javaPlugin) {
        javaPlugin.getCommand("resourcepackmanager").setExecutor(this);
        registerCommands();
    }
    ...
    private void sendMessage(CommandSender commandSender, String message) {
        commandSender.sendMessage("[ResourcePackManager] " + message);
    }
    ...
    public boolean onCommand(CommandSender commandSender, Command cmd, String label, String[] args) {
        ...
        for (AdvancedCommand command : commands) {
            ...
            if (command.onlyForPlayers && !(commandSender instanceof Player)) {
                // Must be a player
                commandSender.sendMessage("[ResourcePackManager] This command must be run as a player!");
                return false;
            }

            if (!((commandSender.hasPermission(command.permission) ||
                    command.permission.equalsIgnoreCase("") ||
                    command.permission.equalsIgnoreCase("resourcepackmanager.")) &&
                    command.enabled)) {
                // No permissions
                commandSender.sendMessage("[ResourcePackManager] You do not have the permission to run this command!");
                return false;
            }
        ...
        commandSender.sendMessage("[ResourcePackManager] Unknown command!");
        return false;
    }
                ...
                for (String alias : command.aliases) {
                    if (alias.toLowerCase().startsWith(args[0].toLowerCase()) && (
                            command.enabled && (commandSender.hasPermission(command.permission)
                                    ...
                                    .equalsIgnoreCase("resourcepackmanager.")))) {
                        result.add(alias);
                    }
                }
            }
