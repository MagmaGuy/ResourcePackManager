 ...
 * rpm-velocity.jar and rpm-bungee.jar from this plugin's resources to
 * {@code plugins/ResourcePackManager/proxy-extension/}. Admins then scp ONE of those
 * to their proxy's plugins/ folder. Backend logs the absolute paths + network-key
 * for easy admin paste.
 *
...
public final class ProxyExtensionExtractor {
    ...
    public static void extractIfNetworkMode(JavaPlugin plugin) {
        ...
        File velocityOut = new File(outDir, "rpm-velocity.jar");
        File bungeeOut = new File(outDir, "rpm-bungee.jar");

        boolean velocityOk = extractOne(plugin, "proxy-extension/rpm-velocity.jar", velocityOut);
        boolean bungeeOk = extractOne(plugin, "proxy-extension/rpm-bungee.jar", bungeeOut);
